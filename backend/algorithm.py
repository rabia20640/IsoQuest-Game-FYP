# Checks if each node follows the color/degree rule #
def check_colour_degree(target_graph, user_graph):
    return True, "colour_degree rule not implemented yet"


# To do: implement rule

# Makes sure nodes are only connected to colours that they are be connected to
...
# Checks if the user graph has the right number of each colour
...
# Compares the edges in the user graph with the target graph
...
# Runs all the rule checking functions one after another and returns all the results
